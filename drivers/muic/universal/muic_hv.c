...
static void max77854_hv_muic_qc_charger(struct hv_data *phv)
{
	struct i2c_client	*i2c = phv->i2c;
#ifdef CONFIG_MUIC_HV_FORCE_LIMIT
	muic_data_t *muic_data = phv->pmuic;
#endif
	int ret = 0;
	u8 status3 =0;
	...
	if(muic_data->pdata->hv_sel) {
		phv->qc_hv = HV_SUPPORT_QC_5V;	// should fix it from DT
	} else {
		phv->qc_hv = HV_SUPPORT_QC_9V;
	}
#endif
...
int hv_set_afc_by_dcm(muic_data_t *pdata, bool onoff)
{
	muic_data_t *muic_data = pdata;
	struct hv_data *phv = muic_data->phv;
	int count = 0;
	...
	if (onoff) {
		...
		case ATTACHED_DEV_AFC_CHARGER_9V_MUIC:
			...
			phv->afc_count = 0;
			max77854_hv_muic_adcmode_switch(phv, true);
			max77854_hv_muic_write_reg(muic_data->i2c, MAX77854_MUIC_REG_HVTXBYTE, 0x0C);
			max77854_hv_muic_afc_control_ping(phv, true);
			break;
		case ATTACHED_DEV_QC_CHARGER_5V_MUIC:
		case ATTACHED_DEV_QC_CHARGER_9V_MUIC:
			...
			max77854_hv_muic_adcmode_switch(phv, true);
			max77854_hv_muic_write_reg(muic_data->i2c, MAX77854_MUIC_REG_HVCONTROL1, 0x33);
			break;
		default:
		...
		case ATTACHED_DEV_AFC_CHARGER_5V_DUPLI_MUIC:
			...
			phv->afc_count = 0;
			max77854_hv_muic_adcmode_switch(phv, true);
			max77854_hv_muic_write_reg(muic_data->i2c, MAX77854_MUIC_REG_HVTXBYTE, 0x46);
			max77854_hv_muic_afc_control_ping(phv, true);
			break;
		case ATTACHED_DEV_QC_CHARGER_5V_MUIC:
			...
			max77854_hv_muic_adcmode_switch(phv, true);
			phv->is_qc_vb_settle = false;
			max77854_hv_muic_write_reg(muic_data->i2c, MAX77854_MUIC_REG_HVCONTROL1, 0x3D);
			break;
		default:
